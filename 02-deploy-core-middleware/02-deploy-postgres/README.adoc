...
[source%linenums,yaml]
----
include::conf/02-deploy-postgres.yaml[]
----
...
----
kubectl get pods,secrets,services,deployments,pvc -n core-middle -l type=postgres
----
...
kubectl exec -it -n core-middle pods/postgres-server-0 -- psql  postgresql://$PG_NAME:$PG_PASSWD@postgres.core-middle.svc.cluster.local:5432 -c "select 1"
----
