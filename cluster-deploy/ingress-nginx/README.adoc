...
[source,bash]
----
wget https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.3.1/deploy/static/provider/cloud/deploy.yaml -O nginx.yaml
mkdir new
cp nginx.yaml new/nginx-public.yaml
cp nginx.yaml new/nginx-private.yaml
----

下载完成后使用如下 Patch 更新配置
...
EOF
kubectl patch service -n ingress-nginx-private ingress-nginx-controller --type merge --patch-file patch.yaml
kubectl patch service -n ingress-nginx-public ingress-nginx-controller --type merge --patch-file patch.yaml
----
