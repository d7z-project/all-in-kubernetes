...
部署完成后，访问 `https://harbor.{{var.global.pub-host}}/` , 使用用户名 `admin` 和配置的密码 `{{var.harbor.admin.password}}` 登陆 *Harbor* 。

=== 配置 LDAP 管理器

访问 `https://harbor.{{var.global.pub-host}}/harbor/configs/auth`,切换验证模式为 LDAP，按下图填入内容。
...
[source%linenums,bash]
----
docker login harbor.{{var.global.pub-host}}
----
