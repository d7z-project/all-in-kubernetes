...
得到密码后，访问 `https://nexus.internal.{{var.global.public.host}}` 来管理。

WARNING: 请将 nexus 的管理员密码设置的足够复杂，以防止被暴力破解！

=== 绑定 LDAP

登陆 Nexus 后，访问 `https://nexus.internal.{{var.global.public.host}}/#admin/security/ldap` ，添加一个 LDAP 服务。具体配置参加如下图片即可。
...
前往 `https://nexus.internal.{{var.global.public.host}}/#admin/security/roles` 页面，对LDAP相关的角色进行管理。
...
前往 `https://nexus.internal.{{var.global.public.host}}/#admin/repository/repositories`，删除默认创建的所有仓库，然后点击新建，选择 `docker (hosted)`, 创建本地私有仓库。具体配置如下：
...
podman tag docker.io/library/nginx:1.21.1 registry.internal.{{var.global.public.host}}/library/nginx:1.21.1
...
前往 `https://nexus.internal.{{var.global.public.host}}/#admin/repository/repositories`，点击新建，选择 `maven2 (hosted)`, 创建本地私有仓库。具体配置如下：
