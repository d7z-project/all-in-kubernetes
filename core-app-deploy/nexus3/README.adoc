...
得到密码后，访问 `https://nexus.internal.{{var.global.pub-host}}` 来管理。

WARNING: 请将 nexus 的管理员密码设置的足够复杂，以防止被暴力破解！

=== 绑定 LDAP

登陆 Nexus 后，访问 `https://nexus.internal.{{var.global.pub-host}}/#admin/security/ldap` ，添加一个 LDAP 服务。具体配置参加如下图片即可。
...
[source,ldif]
----
(|(memberOf=cn=admin,ou=groups,dc=cluster,dc=local)(memberOf=cn=docker,ou=groups,dc=cluster,dc=local)(memberOf=cn=deploy,ou=groups,dc=cluster,dc=local)(memberOf=cn=maven,ou=groups,dc=cluster,dc=local)(memberOf=cn=yum,ou=groups,dc=cluster,dc=local)(memberOf=cn=apt,ou=groups,dc=cluster,dc=local))
----
...
前往 `https://nexus.internal.{{var.global.pub-host}}/#admin/security/roles` 页面，对LDAP相关的角色进行管理。
...
绑定Maven角色的参考如下，注意配置权限：

image::images/02.d-bind-maven-roles.png[title="绑定Maven角色"]

== 创建仓库

=== 创建 Docker 仓库

前往 `https://nexus.internal.{{var.global.pub-host}}/#admin/repository/repositories`，删除默认创建的所有仓库，然后点击新建，选择 `docker (hosted)`, 创建本地私有仓库。具体配置如下：
...
----
# 登陆 docker
podman login registry.internal.{{var.global.pub-host}}
podman pull docker.io/library/nginx:1.21.1
podman tag docker.io/library/nginx:1.21.1 registry.internal.{{var.global.pub-host}}/library/nginx:1.21.1
podman push registry.internal.{{var.global.pub-host}}/library/nginx:1.21.1
----
...
前往 `https://nexus.internal.{{var.global.pub-host}}/#admin/repository/repositories`，点击新建，选择 `maven2 (hosted)`, 创建本地私有仓库。具体配置如下：

image::images/04.a-create-m2-host.png[]

本地仓库创建完成后，即可开始创建仓库代理，这里使用的是官方 `maven` 仓库。

image::images/04.b-create-m2-proxy-default.png[]

最后，组合所有仓库到同一个 Group。

image::images/04.c-create-m2-group.png[]
