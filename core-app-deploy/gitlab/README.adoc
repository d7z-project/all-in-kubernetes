...
*Gitlab* 被配置为只有归属于 `cn=develop` 的用户和管理员才能登陆，需要在 *LDAP* 下创建名为 `gitlab` 的 `groupOfNames` 用户组。
...
创建一个测试用户，并在 `cn=gitlab` 下添加对此用户的绑定，然后在 *Gitlab* 测试登陆，登陆成功后，退出用户并在 *LDAP* 下删除此用户，然后再次测试，如无法登陆则表明部署成功。
