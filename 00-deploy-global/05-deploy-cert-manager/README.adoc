...
crictl pull emberstack/kubernetes-reflector:6.1.47 # <1>
----

.其中
<1> `emberstack/kubernetes-reflector` 用于在多命名空间下同步数据
...
----
include::./conf/02-dnsapi-cloudflare-apikey.yaml[]
----
...
cert-d7z-net       True    d7z-net-tls            18h
----

WARNING: 注意，需要填充的内容必须真实有效，否则将导致签发失败。
