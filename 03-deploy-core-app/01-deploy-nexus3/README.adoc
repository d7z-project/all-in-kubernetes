...
[source%linenums,yaml]
----
include::./conf/01-create-nexus-nginx-conf.yaml[]
----
...
[source%linenums,yaml]
----
include::conf/02-create-nexus-app.yaml[]
----
...
[source%linenums,yaml]
----
include::conf/03-create-nexus-service.yaml[]
----
...
[source%linenums,yaml]
----
include::conf/04-create-nexus-ingress.yaml[]
----
...
----
kubectl exec -it -n core-app pods/nexus3-server-0  --container nexus3-server  -- cat /nexus-data/admin.password
----

访问 `https://nexus.d7z.net` 来管理。
...
在添加绑定前，你需要在 LDAP 下添加 `cn=docker,ou=groups,dc=d7z,dc=net` 的用户组，类型为 `groupOfNames` 。

image::images/01.a-bind-ldap.png[title="配置LDAP连接"]

其中，密码为 LDAP 用户 `service-account` 的密码。
...
----
(|(memberOf=cn=admin,ou=groups,dc=d7z,dc=net)(memberOf=cn=docker,ou=groups,dc=d7z,dc=net))
----
