...
Gitlab CI 需要指定脚本配置部分配置才可正常运行，此处脚本参考至 link:https://yuyy.info/?p=1921[Gitlab CI/CD 实践一：Gitlab Runner 安装到 K8S 集群]。
...
WARNING: *Docker dind* 以特权模式运行容器, 可能会导致严重的安全问题。建议配置节点亲和性使其运行在专门的节点中。
